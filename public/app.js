...
const thinkingInput = document.querySelector("#thinking");
...
temperatureInput.addEventListener("input", () => {
  temperatureOutput.value = Number(temperatureInput.value).toFixed(2);
});
...
async function refreshModels() {
  setStatus("Checking ACE-Step...", "pending");
  ...
  try {
    ...
    if (!response.ok || !payload.ok) {
      throw new Error(payload.error || "ACE-Step is not responding.");
    }

    const defaultModel = payload.models?.defaultModel || "";
    const models = payload.models?.items || [];

    modelSelect.innerHTML = "";

    const defaultOption = document.createElement("option");
    defaultOption.value = "";
    defaultOption.textContent = defaultModel ? `Default (${defaultModel})` : "Default";
    modelSelect.append(defaultOption);

    for (const model of models) {
      const option = document.createElement("option");
      option.value = model.name;
      option.textContent = model.isDefault ? `${model.name} (default)` : model.name;
...
async function generateAudio() {
  setStatus("Submitting ACE-Step task...", "pending");
  ...
  try {
    const response = await fetch("/api/generate", {
      ...
      body: JSON.stringify({
        ...
        inferenceSteps: Number(stepsInput.value),
        temperature: Number(temperatureInput.value),
        thinking: thinkingInput.checked
      })
    });
    ...
    showResult(payload);
    setStatus("Music generated by ACE-Step", "ok");
  } catch (error) {
    setStatus(error.message, "error");
  } finally {
...
function showResult(payload) {
  ...
  resultTitle.textContent = payload.plan.title || "Generated music";
